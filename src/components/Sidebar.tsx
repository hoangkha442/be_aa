    ...
    >
      <TooltipProvider delayDuration={150}>
        <div className={`${isCollapsed ? "p-1" : "p-3"}`}>
          <nav className={`flex flex-col ${isCollapsed ? "justify-center space-y-1.5 mt-2" : "gap-1"}`}>
            {items.map((it) => {
              const Icon = it.icon;
              const active = isActivePrefix(it.to)
              const link = (
                <NavLink
                  key={it.to}
                  to={it.to}
                ...
                >
                  <div className={`${isCollapsed ? "w-full! flex justify-center py-2" : " "} ${active ? "text-slate-900 bg-muted!  rounded-xl" : "text-muted-foreground"}`}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="outline"
                  className={cn(
                    "w-full h-10  cursor-pointer bg-slate-700 text-slate-100 hover:bg-slate-900! hover:text-slate-50! transition-all duration-500",
                    isCollapsed ? "px-0 justify-center" : "justify-start gap-2"
                  )}
                  onClick={logout}
                >
